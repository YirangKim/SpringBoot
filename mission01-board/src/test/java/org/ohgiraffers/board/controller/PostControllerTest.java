...
/* 통합테스트 단위테스트
* 통합테스트 :
* 모듈을 통합하는 과정에서 묘듈간의 호환성을 확인하기 위해 수행된느 테스트
* 통합테스트는 API의 모든 과정이 올바르게 동작하는지 확인하는 것
*
* 단위(유닛)테스트 :
* 하나의 모듈을 기준으로 독립적으로진행되는 가장 작은 단위 테스트 (함수, 메소드)
* 단위테스트는 어플리케이션을 구성하는 하나의 기능이 올바르게 동작하는지 독립적으로 테스트 하는 것
* */

/* @WebMvcTest
* MVC를 위한  테스트, 컨트롤럴가 예상대로 동작하는지 테스트하는데 사용된다
...
import org.ohgiraffers.board.controller.PostController;
...
public class PostControllerTest {
    ...
    * */
    ...
    /** MockBean
    * 스프링의 의존성 주입기능을 사용해 테스트 실행 시 실제 번 대신 Mockito로 생성된
    ...
    * */
    ...
    void create_post_test() throws Exception{
        ...
        // any 아무거나 넣으면 response로 return, createPost 가짜 객체
        given(postService.createPost(any())).willReturn(response);

        //when & then
        mockMvc.perform(post("/api/v1/posts")
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsBytes(request)) //request담아서 보내준다
        )
